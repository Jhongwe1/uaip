...
// 啟用中的 VPN 渠道列（vpn_channels 表；url/nodes 欄位 schema 預設空字串）
interface VpnChRow {
  kind: string;
  url: string;
  nodes: string;
...
function nodeLines(text: string | null | undefined): string[] {
  if (!text) return [];
  return text
    .split(/\r?\n/)
    .map(function (s) {
...
async function fetchNodes(url: string, ua: string): Promise<string[] | null> {
