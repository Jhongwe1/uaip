...
export async function onRequest(context: RouteCtx): Promise<Response> {
  ...
  try {
    ...
    ch = (((res[0].results || [])[0] as unknown as ChannelRow | undefined) || null) as ChannelRow | null;
    const m = (res[1].results || [])[0] as { v?: string } | undefined;
  ...
  const enc = function (s: string): string {
