...
export async function onRequestPost({ request, env }: RouteCtx): Promise<Response> {
  ...
  await env.DB.prepare(
    ...
    .run();
  const user = await env.DB.prepare("SELECT * FROM users WHERE google_sub=?1").bind(sub).first<UserRow>();

  const sess = await createSession(env, user as UserRow, url);
...
function devForm(next: string, err?: string): Response {
  return miniPage(
    "本機測試登入",
