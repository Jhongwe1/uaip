...
// 驗證：Authorization: Bearer <LOGS_TOKEN>；LOGS_TOKEN 是 Worker 的加密環境變數
//（用 `npx wrangler secret put LOGS_TOKEN` 設定/更換）。
...
// 2026-07-11 起：管理員 Google 帳號的登入 cookie 也可以（與金鑰並存），驗證邏輯統一在 src/lib/auth.ts
...
export async function onRequestGet({ request, env }: RouteCtx): Promise<Response> {
  ...
  try {
    ...
    const out: Record<string, unknown> = {
      rows: res[0].results,
      total: (res[1].results[0] as { c?: unknown }).c
    };
    if (since) {
      out.today = (res[2].results[0] as { c?: unknown }).c;
      out.todayIps = (res[2].results[0] as { ips?: unknown }).ips;
    }
    return json(out);
  } catch (e: any) {
