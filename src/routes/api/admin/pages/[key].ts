...
async function findPage(env: Env, key: string | string[] | undefined): Promise<PageDbRow | null | undefined> {
  const k = String(key || "");
  if (/^\d+$/.test(k)) {
    return env.DB.prepare("SELECT * FROM pages WHERE id=?1").bind(parseInt(k, 10)).first<PageDbRow>();
  }
  const slug = k.toLowerCase();
  if (!SLUG_RE.test(slug)) return undefined;
  return env.DB.prepare("SELECT * FROM pages WHERE slug=?1").bind(slug).first<PageDbRow>();
}
