...
export async function onRequestPut(context: RouteCtx): Promise<Response> {
  ...
  if (items.length > MAX_ITEMS)
    return json({ error: "too-many", hint: "選單最多 " + MAX_ITEMS + " 項" }, 400);

  const clean: { kind: string; label: string; label_en: string; url: string }[] = [];
  for (let i = 0; i < items.length; i++) {
    ...
    const label_en = String(it.label_en == null ? "" : it.label_en)
      .trim()
      .slice(0, 60);
    let u = "";
    if (kind === "link") {
      u = String(it.url == null ? "" : it.url)
        .trim()
        .slice(0, 300);
  ...
  try {
    ...
    clean.forEach(function (c, i) {
      stmts.push(
        env.DB.prepare("INSERT INTO menu (pos, kind, label, label_en, url) VALUES (?1,?2,?3,?4,?5)").bind(
          i,
          c.kind,
          c.label,
          c.label_en,
