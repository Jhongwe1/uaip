...
const HINT_ADM = "ipua_adm"; // 非 HttpOnly 提示：是管理員 → 前端才載入 adminbar.js
...
export function randToken(prefix?: string, len?: number): string {
...
function cookieStr(
  ...
  value: string,
  maxAge: number,
  opts?: { path?: string; httpOnly?: boolean; secure?: boolean }
): string {
  opts = opts || {};
  let s =
    name +
...
export async function createSession(
  ...
  user: UserRow,
  url: URL
): Promise<{ sid: string; cookies: string[] }> {
...
export function clearSessionCookies(url: URL): string[] {
  const secure = url.protocol === "https:";
  return [
...
/* ===== 分服務批准（2026-07-13）=====
   管理員可以對每個會員「分別」批准不同服務：users.services 存逗號分隔清單。
   管理員帳號不看 services、全部服務都能用；封鎖一律全擋。 */
...
export function hasService(user: UserRow | null | undefined, env: Env, svc: string): boolean {
  return userServices(user, env).indexOf(svc) >= 0;
}

/* ===== Playground 全員開放開關（2026-07-14）=====
   管理員不缺 token 時可以整站開放：settings 表 pg_open='1' ＝ 任何已登入、
   未封鎖的會員都能用 playground，不必逐人批准；沒這個鍵＝關（維持逐人批准）。
   只影響 playground — relay 與 vpn 照舊看個人批准。 */
export async function pgOpenAll(env: Env): Promise<boolean> {
  if (!env || !env.DB) return false;
  try {
    const r = await env.DB.prepare("SELECT v FROM settings WHERE k='pg_open'").first<{ v: string }>();
    return !!(r && r.v === "1");
  } catch (e) {
    return false;
  }
...
export async function canUsePlayground(user: UserRow | null | undefined, env: Env): Promise<boolean> {
...
export function memberKeyFrom(request: Request, url: URL): string {
  const auth = request.headers.get("authorization") || "";
...
// 用會員金鑰換會員資料；無效回 null。
export async function userFromKey(env: Env, key: string): Promise<UserRow | null> {
...
export function miniPage(title: string, bodyHtml: string, status?: number): Response {
  const html =
