// src/lib/pages.ts — 列表頁與文章頁的實際內容（src/routes/news、src/routes/articles 共用）。
// 列表排版仿 antutu 新聞列表：左縮圖、右標題＋兩行摘要＋「N 小時前・瀏覽數」，底部頁碼。
// 文章頁由伺服器把 Markdown 轉成完整 HTML 再輸出 — 搜尋引擎收錄與 LINE/FB 分享預覽全靠這個。

import { marked } from "./vendor/marked.mjs";
import { sanitizeHtml } from "./sanitize.js";
import { siteOrigin, CATS, esc, html, timeAgo, pageShell, EYE } from "./site.js";
import type { Cat, CatKey, Chrome } from "./site.js";
import { getChromeFor } from "./chrome.js";
import type { ArticleRow, RouteCtx } from "../types.js";

// 列表查詢只 SELECT 這幾欄（不是 articles 整列）
interface ListRow {
  id: number;
  title: string;
  summary?: string;
...
export async function listPage(context: RouteCtx, catKey: CatKey): Promise<Response> {
  ...
  const chromeP = getChromeFor(env, request); // 選單＋站名（依身分過濾），與文章查詢並行
  let rows: ListRow[] = [],
    total = 0;
  try {
    const res = await env.DB.batch([
      ...
      env.DB.prepare("SELECT COUNT(*) AS c FROM articles WHERE status='published' AND category=?1").bind(
        catKey
      )
    ]);
    rows = (res[0].results || []) as unknown as ListRow[];
    total = ((res[1].results[0] || {}) as { c?: number }).c || 0;
  } catch (e) {
    /* 資料表尚未建立時顯示空列表，不讓整頁掛掉 */
  }

  const pages = Math.max(1, Math.ceil(total / PER_PAGE));
  const items = rows
    .map(function (r) {
      return itemHtml(cat, r);
    })
    .join("\n");
  const body =
    '<section class="list">\n' +
  ...
  return html(
    pageShell({
      ...
      canonical: siteOrigin(env, request) + cat.path + (p > 1 ? "?p=" + p : ""),
      activePath: cat.path,
      chrome: chrome,
...
function itemHtml(cat: Cat, r: ListRow): string {
...
function pagerHtml(base: string, p: number, pages: number): string {
  if (pages <= 1) return "";
  const link = function (n: number, text?: string, key?: string): string {
    const href = base + (n > 1 ? "?p=" + n : "");
    return n === p && !text
      ? '<span class="cur">' + n + "</span>"
      : '<a href="' + href + '"' + (key ? ' data-i18n="' + key + '"' : "") + ">" + (text || n) + "</a>";
  };
  const want = [1, 2, p - 1, p, p + 1, pages - 1, pages].filter(function (n) {
    return n >= 1 && n <= pages;
  });
  const ns = Array.from(new Set(want)).sort(function (a, b) {
    return a - b;
  });
  let mid = "",
    prev = 0;
  ...
  return (
    ...
    (p > 1 ? link(p - 1, "‹ 上一頁", "pg.prev") : "") +
    mid +
    (p < pages ? link(p + 1, "下一頁 ›", "pg.next") : "") +
    "</nav>"
  );
}

/* ===== 文章頁：/news/12、/articles/34 ===== */
export async function articlePage(context: RouteCtx, catKey: CatKey): Promise<Response> {
  ...
  if (id > 0 && env.DB) {
    try {
      row = await env.DB.prepare("SELECT * FROM articles WHERE id=?1 AND category=?2 AND status='published'")
        .bind(id, catKey)
        .first<ArticleRow>();
    } catch (e) {}
  }
  if (!row) return notFound(cat, (await chromeP).chrome);
  ...
  try {
    ...
    if (
      request.method === "GET" &&
      !isBot &&
      purpose.indexOf("prefetch") < 0 &&
      purpose.indexOf("preview") < 0
    ) {
      context.waitUntil(
        env.DB.prepare("UPDATE articles SET views=views+1 WHERE id=?1")
          .bind(id)
          .run()
          .catch(function () {})
      );
  ...
  let older: { id: number; title: string } | null = null,
    newer: { id: number; title: string } | null = null;
  try {
    ...
    older = ((nav[0].results || [])[0] as { id: number; title: string } | undefined) || null;
    newer = ((nav[1].results || [])[0] as { id: number; title: string } | undefined) || null;
  } catch (e) {}

  const chrome = (await chromeP).chrome;
  const origin = siteOrigin(env, request);
  const canonical = origin + cat.path + "/" + row.id;
  const coverAbs = row.cover ? (row.cover.charAt(0) === "/" ? origin + row.cover : row.cover) : "";
  // Markdown → HTML 後過白名單消毒（marked 會放行原始 HTML — 見 lib/sanitize.js）；
    ...
    anav =
      ...
      (older
        ? '<a href="' +
          ...
          "/" +
          older.id +
          '"><span class="lbl" data-i18n="an.prev">上一篇</span><span class="ttl">' +
          esc(older.title) +
          "</span></a>"
        : "<span></span>") +
      (newer
        ? '<a class="nx" href="' +
          ...
          "/" +
          newer.id +
          '"><span class="lbl" data-i18n="an.next">下一篇</span><span class="ttl">' +
          esc(newer.title) +
          "</span></a>"
        : "") +
      "</nav>";
  ...
  const body =
    ...
    esc(row.title) +
    "</h1>\n" +
    '<div class="ameta"><span data-i18n="' +
    cat.tkey +
    '">' +
    cat.label +
    "</span>" +
    ...
    "\n</div>\n" +
    anav +
    '<div class="backrow"><a class="ctrl" href="' +
    cat.path +
    '" data-i18n="back.' +
    cat.key +
    '">← 回' +
...
function notFound(cat: Cat, chrome: Chrome): Response {
  return html(
    pageShell({
      title: "找不到內容",
      desc: "這篇內容不存在或已下架。",
      ...
      body:
        ...
        '<div class="backrow" style="text-align:center"><a class="ctrl" href="' +
        (cat ? cat.path : "/") +
        '"' +
        (cat ? ' data-i18n="back.' + cat.key + '"' : "") +
        ">← 回" +
        (cat ? cat.label + "列表" : "首頁") +
        "</a></div>"
    }),
