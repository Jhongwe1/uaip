...
describe("relay 安全邊界", () => {
  ...
  it("沒被批准 relay 服務 → 403；封鎖 → 403", async () => {
    for (const over of [
      { status: "approved", services: "vpn" },
      { status: "blocked", services: "relay" }
    ]) {
      const u = await seedUser(over);
      const key = await giveKey(u);
      const ctx = await relayCtx("/relay/up/v1/x", { headers: { authorization: "Bearer " + key } }, [
        "up",
        "v1",
        "x"
      ]);
...
describe("relay 轉發：標頭剝除＋金鑰置換", () => {
  ...
  it("anthropic 原生路徑用 x-api-key；gemini 原生用 x-goog-api-key", async () => {
    ...
    const ctx2 = await relayCtx(
      "/relay/gem/v1beta/models/gemini-x:generateContent",
      {
        method: "POST",
        headers: { authorization: "Bearer " + key, "content-type": "application/json" },
        body: "{}"
  ...
  it("OpenAI 相容路徑（gemini 渠道的 /openai/）統一用 Bearer", async () => {
    ...
    const cap = interceptUpstream({ body: "{}" });
    const ctx = await relayCtx(
      "/relay/gemc/v1beta/openai/chat/completions",
      {
        method: "POST",
        headers: { authorization: "Bearer " + key, "content-type": "application/json" },
        body: "{}"
...
describe("relay 回應直通", () => {
  ...
  it("記用量：成功轉發後 relay_calls +1", async () => {
    ...
    const ctx = await relayCtx("/relay/up/v1/models", { headers: { authorization: "Bearer " + key } }, [
      "up",
      "v1",
      "models"
    ]);
