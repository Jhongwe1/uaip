...
- Nothing persists (no conversation rows); accounting goes to a synthetic `demo:public` user row that can never log in.
...
fail-open 刻意相反。對話不落地；記帳走永遠無法登入的 `demo:public` 合成帳號。最壞日燒錢
