...
(function(){
    ...
    api("/api/admin/relay/channels").then(function(d){
      ...
      rows.forEach(function(c){
        ...
        tg.addEventListener("click",function(){
          tg.disabled=true;
          api("/api/admin/relay/channels/"+c.id,{method:"PUT",json:{name:c.name,slug:c.slug,kind:c.kind,base_url:c.base_url,enabled:!c.enabled}})
  ...
  function editChannel(c,box){
    ...
    // 只在「欄位是空的」或「裡面還是某個官方預設值」時才覆蓋 — 站長手打過的網址絕不動。
    ...
    save.addEventListener("click",function(){
      var payload={name:fName.value.trim(),slug:fSlug.value.trim().toLowerCase(),kind:sel.value,base_url:fBase.value.trim(),enabled:isNew?1:!!c.enabled};
