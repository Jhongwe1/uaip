...
// 站長另外看到「管道管理」卡：新增／編輯／刪除上游管道（存 relay_channels 表）。
// 真正的轉發在 functions/relay/[[path]].js；這頁只是操作面板，所有寫入都打 API。
...
(function(){
  ...
  function start(){
    MU.me(true).then(function(u){
      me=u;
      if(!me){MU.gateLogin(root,tx("API 中轉站","API relay"),tx("用一把金鑰接上各家 AI API（OpenAI、Claude、Gemini、本地模型…）。請先用 Google 登入。","One key for many AI APIs. Please sign in with Google first."));return;}
      if(!me.approved){MU.gatePending(root,me);return;}
      return fetch("/api/relay/channels",{cache:"no-store"}).then(function(r){return r.json();}).then(function(d){
        channels=d.rows||[];render();
      });
  ...
  function render(){
    ...
    if(!channels.length){
      cc.appendChild(el("p","muted",tx("站長還沒設定任何上游管道。","No upstream channels configured yet.")));
    }else{
  ...
  function adminCard(){
    var card=el("div","card");
    var h=el("h2");h.appendChild(document.createTextNode(tx("管道管理（站長）","Channels admin")));
  ...
  function editChannel(c,box){
    ...
    var fSlug=field(tx("網址代稱（slug）","Slug"),"cSlug",c.slug,"openai");
    if(!isNew)fSlug.disabled=false;
    // kind
    var kf=el("div","field");kf.appendChild(el("label",null,tx("類型（決定金鑰怎麼帶給上游）","Kind")));
    var sel=el("select");["openai","anthropic","gemini","custom"].forEach(function(k){
      var o=el("option",null,k);o.value=k;if(c.kind===k)o.selected=true;sel.appendChild(o);
    ...
    function upHint(){
      var m={openai:"https://api.openai.com（相容服務填它的網址，含本地 http://…）",anthropic:"https://api.anthropic.com",gemini:"https://generativelanguage.googleapis.com",custom:"任何 OpenAI 相容網址"};
      hint.textContent=tx("Base URL 例：","e.g. ")+(m[sel.value]||"");
    }
    sel.addEventListener("change",upHint);upHint();
    ...
    save.addEventListener("click",function(){
      var payload={name:fName.value.trim(),slug:fSlug.value.trim().toLowerCase(),kind:sel.value,base_url:fBase.value.trim(),enabled:1};
