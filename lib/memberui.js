...
(function(){
  ...
  var lang="zh";try{if(localStorage.getItem("ipua-lang")==="en")lang="en";}catch(e){}
  ...
  function badge(user){
    ...
    if(user.is_admin){var a=el("span","badge ok",tx("站長","Admin"));return[a,b];}
    return[b];
  }
  ...
  function gatePending(host,user){
    ...
    var g=el("div","gate");
    g.appendChild(el("div","big","\\u23F3"));
    g.appendChild(el("h2",null,tx("等待站長核准","Waiting for approval")));
    g.appendChild(el("p",null,tx("你已經用 "+user.email+" 登入了，但這個功能需要站長核准帳號後才能使用。核准後回來重新整理即可。","You're signed in as "+user.email+", but this feature needs the site owner to approve your account first.")));
    host.appendChild(g);
  }
  function flash(msg){
    var f=$("mu-flash");
  ...
  window.MU={$:$,el:el,esc:esc,tx:tx,me:me,statusText:statusText,badge:badge,acctCard:acctCard,
