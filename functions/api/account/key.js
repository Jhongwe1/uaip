...
import { getSessionUser, goodOrigin, randToken, sha256hex, keyHint, json } from "../../../lib/auth.js";
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    await env.DB.prepare("UPDATE users SET api_key_hash=?1, api_key_hint=?2, api_key_at=?3 WHERE id=?4")
      .bind(await sha256hex(key), keyHint(key), now, user.id)
      .run();
    return json({
      key: key,
...
export async function onRequestDelete({ request, env }) {
  ...
  try {
    await env.DB.prepare("UPDATE users SET api_key_hash='', api_key_hint='', api_key_at=NULL WHERE id=?1")
      .bind(user.id)
      .run();
    return json({ ok: true });
  } catch (e) {
